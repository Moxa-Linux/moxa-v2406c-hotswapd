...
ifeq ($(OS),CentOS)
	/usr/bin/install -m 644 -D libmxhtsp.so /usr/lib64/libmxhtsp.so
	/usr/bin/install -m 644 -D 96-moxa-disk-centos.rules /etc/udev/rules.d/96-moxa-disk-centos.rules
else
